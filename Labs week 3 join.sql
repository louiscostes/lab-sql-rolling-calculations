...
-- total amount rung up by each staff member in augst 2005
...
-- display each film and number of actors who are listed for that film
...
from sakila.store
join (sakila.address join (sakila.city join sakila.country 
using (country_id)) using (city_id)) using (address_id);
...
use sakila;
 ...
 select fa1.film_id, concat(a1.first_name, ' ', a1.last_name) as 'actor1',
 concat(a2.first_name, ' ', a2.last_name) as 'actor2'
 from sakila.actor as a1 
 inner join film_actor as fa1 
 on a1.actor_id = fa1.actor_id
 inner join film_actor as fa2
 on (fa1.film_id = fa2.film_id) and (fa1.actor_id !=
 fa2.actor_id)
...
select count(distinct customer_id) as Active_users, Activity_year,
Activity_Month
from customer_activity
group by Activity_year, Activity_Month
order by Activity_year, Activity_Month
...
last_month, Activity_year, Activity_month
...
-- % change in number of active customers
with customer_activity as (
  select customer_id, convert(rental_date, date) as Activity_date,
  date_format(convert(rental_date,date), '%M') as Activity_Month,
  date_format(convert(rental_date,date), '%Y') as Activity_year
